...
The add-in opens a compact task pane UI for PowerPoint. Copy a selected image in PowerPoint, paste it into the pane with Ctrl+V, then use `모두 PPT에 넣기` to insert the trimmed result into the current slide. When there are multiple results, `모두 복사` and `모두 PPT에 넣기` use one combined PNG. File selection is still available for images outside the presentation.
